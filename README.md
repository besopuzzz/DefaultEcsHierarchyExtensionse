...
Используйте проект как идею для создания своей иерархии (уверен, профессионалы сделают лучше) или как источник (https://www.nuget.org/packages/DefaultEcs.Hierarchy).
...
- Вызов Dispose() у сущности так же вызывает Dispose() у дочерних сущностей.
- Инициализация нескольких TreeReference по одному ключу TKey ссылаются на один экземпляр Tree<TKey>. Этого говорит о том, что несколько используемых TreeReference не создают дополнительные ресурсные расходы.

# Планы
- Сделать конструктор класса Trees публичным и отдать управление жизнью класса в руки пользователя. 
